...
const week7Dir = path.join(baseDir, '..');
const sharedDir = path.join(week7Dir, '..', 'shared_context');

function getCompiledPrompt() {
    const roleFile = path.join(sharedDir, 'role.md');
    const architectureFile = path.join(sharedDir, 'architecture_framework.md');
    const twelveFactorFile = path.join(sharedDir, 'twelve_factor_app_guide.md');
    ...
    const promptFile = path.join(baseDir, 'prompt_assignment7.md');

    const roleText = fs.readFileSync(roleFile, 'utf8');
    const architectureText = fs.readFileSync(architectureFile, 'utf8');
    const twelveFactorText = fs.readFileSync(twelveFactorFile, 'utf8');
...
--- GUÍA MAESTRA: THE TWELVE-FACTOR APP (12-FACTOR.NET) ---
${twelveFactorText}
...
=========================================
ASSIGNMENT 7: DOMINIO DE PEDIDOS GUIADO POR EVENTOS
=========================================
${problemText}
...
async function generate() {
    try {
        console.log(`[INFO] Compilando prompt de Assignment 7 (Patrones C4, Database-per-Service y DDD) en memoria...`);
        ...
        while ((finishReason === 'MAX_TOKENS' || !fullText.includes('Checklist') && !fullText.includes('Perfilado de Carga')) && attempts < 3) {
            attempts++;
...
Has estado generando la solución arquitectónica para Assignment 7 (Patrones C4, Database-per-Service y DDD).
Tu última respuesta finalizó en el siguiente fragmento:

"${lastSnippet}"

CONTINÚA LA GENERACIÓN EXACTAMENTE DESDE EL PUNTO DONDE TE QUEDASTE. Completa las secciones restantes (Diseño Táctico DDD, Diagrama de Agregados, Perfilado de Carga y Checklist) hasta finalizar completamente el documento.
`;
        ...
        const outputFile = path.join(baseDir, 'assignment7_output.md');
        fs.writeFileSync(outputFile, fullText, 'utf8');

        console.log(`[ÉXITO] Assignment 7 generado e inscrito exitosamente en: ${outputFile} (${fullText.length} caracteres).`);
    } catch (error) {
