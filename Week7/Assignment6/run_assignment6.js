...
function getCompiledPrompt() {
    ...
    const promptFile = path.join(baseDir, 'prompt_assignment6.md');
...
=========================================
ASSIGNMENT 6: DOMINIO DE PEDIDOS GUIADO POR EVENTOS
=========================================
${problemText}
...
async function generate() {
    try {
        console.log(`[INFO] Compilando prompt de Assignment 6 (Validación C4, Elasticidad y Ensayo Prime Video) en memoria...`);
        ...
        while ((finishReason === 'MAX_TOKENS' || !fullText.includes('Checklist') && !fullText.includes('Prime Video')) && attempts < 3) {
            attempts++;
...
Has estado generando la solución arquitectónica para Assignment 6 (Validación C4, Elasticidad y Ensayo Prime Video).
Tu última respuesta finalizó en el siguiente fragmento:

"${lastSnippet}"

CONTINÚA LA GENERACIÓN EXACTAMENTE DESDE EL PUNTO DONDE TE QUEDASTE. Completa las secciones faltantes (Ensayo de Prime Video y Checklist) hasta finalizar completamente el documento.
`;
        ...
        const outputFile = path.join(baseDir, 'assignment6_output.md');
        fs.writeFileSync(outputFile, fullText, 'utf8');

        console.log(`[ÉXITO] Assignment 6 generado e inscrito exitosamente en: ${outputFile} (${fullText.length} caracteres).`);
    } catch (error) {
