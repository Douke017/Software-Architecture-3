# Estándar General de Análisis y Diseño de Arquitectura de Microservicios Distribuidos (Senior Architect Framework)

Este marco metodológico define el procedimiento general que debe aplicar el **Principal Software Architect** para descomponer, analizar y resolver problemas de arquitectura de software a gran escala (monolitos acoplados, sistemas de alto volumen y ecosistemas multi-canal).
...
1. **Aislamiento de Dominios**: Identificación clara de contextos delimitados (ej. *Order Service*, *Inventory Service*, *Payment Service*, *Customer/Loyalty Service*, *Notification Service*).
...
1. **Comunicación Sincrónica Estricta (gRPC / REST + Circuit Breaker)**:
   - Reservada exclusivamente para consultas de lectura de baja latencia o validaciones externas inmediatas (ej. autorización en pasarela de pagos).
   - Debe protegerse obligatoriamente mediante patrones de **Circuit Breaker** (ej. Resilience4j), **Timeouts** y **Bulkheads** para evitar el agotamiento de hilos (*thread pool starvation*).
2. **Comunicación Asincrónica Guiada por Eventos (EDA & Event Bus)**:
   - Obligatoria para todas las operaciones que modifiquen el estado del negocio (`OrderPlaced`, `InventoryReserved`, `PaymentProcessed`).
...
2. **Estrategia de Caché Distribuido (Redis Cluster)**:
   - Capa de almacenamiento en memoria para absorber >90% de las lecturas repetitivas (catálogos, precios, ofertas) liberando la capa de datos relacional.
